def generate_html():
    ...
    * {margin:0; padding:0; box-sizing:border-box;}
    ...
    .pnl-fun {background: linear-gradient(135deg, #FFE0B2 0%, #FFCC80 100%); border:none; box-shadow: 0 15px 35px rgba(0,0,0,0.1); border-radius:30px; padding: 40px; display:flex; align-items:center; gap:30px;}
    ...
    def rep_slide(n, d, text, svgs):
        return f"""
        <div class="head-title">Representaciones</div>
    ...
    def draw_painted(typ, col, painted_cnt):
        ...
        if typ == "grid2x3":
            for i in range(2):
                for j in range(3):
                    idx = i*3 + j
                    s += f'<rect x="{j*80}" y="{i*120}" width="80" height="120" fill="{col if idx<painted_cnt else "#fff"}" stroke="#555" stroke-width="3"/>'
        elif typ == "grid2x4":
            for i in range(2):
                for j in range(4):
                    idx = i*4 + j
                    s += f'<rect x="{j*60}" y="{i*120}" width="60" height="120" fill="{col if idx<painted_cnt else "#fff"}" stroke="#555" stroke-width="3"/>'
        elif typ == "circle6":
            ...
            for i, p in enumerate(paths): s += f'<path d="{p}" fill="{col if i<painted_cnt else "#fff"}" stroke="#555" stroke-width="2"/>'
        elif typ == "circle4":
            ...
            for i in range(3): s += f'<rect x="{i*80}" y="20" width="80" height="200" fill="{col if i<painted_cnt else "#fff"}" stroke="#555" stroke-width="3"/>'
        s += '</svg>'
        return s

    def custom_rect_frac(den, num, col, mw=180):
        w = mw / den
        s = f'<svg width="{mw+4}" height="60" viewBox="-2 -2 {mw+4} 64">'
        for i in range(den):
            fill = col if i < num else "#fff"
            s += f'<rect x="{i*w}" y="0" width="{w}" height="60" fill="{fill}" stroke="#555" stroke-width="3"/>'
        s += '</svg>'
        return s
    ...
    # PPT 4 COMPARACIONES (18-19)
    # Mismo Denominador Visual
    svg_3_8 = draw_painted("grid2x4", "#fc4d51", 3)
    svg_5_8 = draw_painted("grid2x4", "#fc4d51", 5)
    slide(f"""
        ...
        <div class="pnl-fun" style="background: linear-gradient(135deg, #FFDEE9 0%, #B5FFFC 100%);">
            <div style="flex:1;">
                ...
                <div class="stp" style="display:flex; align-items:center; gap:20px; justify-content:center; margin:20px 0;">
                    <div style="text-align:center;">{svg_3_8}<br><b style="font-size:30px;">3/8</b></div>
                    <div style="font-size:50px; font-weight:900; color:var(--base);">VS</div>
                    <div style="text-align:center;">{svg_5_8}<br><b style="font-size:30px;">5/8</b></div>
                </div>

                <div class="stp" style="background:#fff; padding:20px; border-radius:15px; border-left:8px solid #FF6B9D; font-size:22px; font-weight:800;">
                    Mismo denominador = partes del mismo tamaño.<br>
                    Regla: Gana el que tiene más partes pintadas.<br>
    ...
    # Mismo Numerador Visual (Con barras)
    bar_1_3 = custom_rect_frac(3, 1, "#047857", 300)
    bar_1_5 = custom_rect_frac(5, 1, "#047857", 300)
    slide(f"""
        <div class="head-title">Comparar Mismo Numerador</div>
        <div class="pnl-fun" style="background: linear-gradient(135deg, #d4fc79 0%, #96e6a1 100%); flex-direction:column;">
            <p style="font-size:28px; font-weight:800; margin-bottom:10px;">Comparamos <b>1/3</b> y <b>1/5</b></p>
            <div class="stp" style="font-size:22px; text-align:center;">El numerador es igual (1 trozo). ¡Pero mira el tamaño del trozo!</div>
            
            <div class="stp" style="display:flex; flex-direction:column; gap:20px; align-items:center; width:100%;">
                <div style="display:flex; align-items:center; gap:20px;">
            ...
            <div class="stp" style="background:#fff; padding:20px; border-radius:15px; border-left:8px solid #047857; font-size:22px; font-weight:800; width:100%; text-align:center; margin-top:10px;">
                Regla: MENOS cortes en la barra = TROZOS MÁS GORDOS.<br>Gana el Denominador menor.<br>
    ...
    # MCM CONCEPT (20-22)
    slide(f"""
        <div class="head-title" style="background:#F57F17;">¡Problema! Diomas Diferentes</div>
        <div class="pnl" style="border: 5px solid #FBC02D; text-align:center; font-size:26px;">
            <p>¿Qué pasa si queremos comparar <b>2/3</b> y <b>3/4</b>?</p>
            <div class="stp" style="margin-top:20px; font-weight:800; color:#b91c1c;">¡No tienen ni el mismo numerador ni el mismo denominador!</div>
            <div class="stp" style="margin-top:20px; font-size:80px;">🤷🏽‍♂️</div>
            <div class="stp" style="margin-top:20px; font-weight:800; font-size:24px; color:var(--base);">
                Para poder compararlas, tenemos que hacer que hablen el mismo "idioma" (que tengan el mismo denominador).<br><br>
                Para lograr eso, llamamos a un superhéroe: <br><span style="font-size:40px; color:#F57C00;">El Mínimo Común Múltiplo (MCM)</span>.
            </div>
        </div>
    """)
    ...
    slide(f"""
        ...
        <div class="pnl-fun" style="background: linear-gradient(135deg, #FF9A9E 0%, #FECFEF 100%); flex-direction:column; text-align:center;">
            <p style="font-size:26px; font-weight:800;">Mínimo Común Múltiplo (MCM)</p>
            <p style="font-size:22px; margin-top:10px;">Es el número más pequeño que aparece repetido en las tablas de multiplicar de ambos números.</p>
            
            <div class="stp" style="background:#fff; padding:20px; border-radius:20px; width:100%; margin-top:20px;">
                <b style="font-size:24px; color:#d63384;">Ejemplo: Queremos el MCM de 3 y 4</b>
                <div style="font-size:22px; margin-top:15px; text-align:left; display:inline-block;">
                    <b>Tabla del 3:</b> 3, 6, 9, <span style="background:#FFEB3B; padding:5px 10px; border-radius:5px;">12</span>, 15, 18, 21, <span style="background:#FFCC80; padding:5px 10px; border-radius:5px;">24</span>...<br>
                    <b>Tabla del 4:</b> 4, 8, <span style="background:#FFEB3B; padding:5px 10px; border-radius:5px;">12</span>, 16, 20, <span style="background:#FFCC80; padding:5px 10px; border-radius:5px;">24</span>...
                </div>
            </div>
            
            <div class="stp" style="margin-top:20px; font-size:30px; font-weight:900; color:#c2185b;">
                Tienen muchos múltiplos en común (12, 24...), ¡pero el MÍNIMO (el primero) es el 12!
            </div>
        </div>
    """)
    ...
    slide(f"""
        ...
        <div class="pnl" style="font-size:26px; text-align:center; border: 4px dashed #9de3bd;">
            Encontremos el MCM de <b>2</b> y <b>5</b>.
            
            <div class="stp" style="margin-top:20px; display:flex; flex-direction:column; gap:10px;">
                <div style="background:#f8f9fa; padding:15px; border-radius:10px;">
                    <b>Múltiplos de 2:</b> 2, 4, 6, 8, <span class="stp hl">10</span>, 12...
                </div>
                <div style="background:#f8f9fa; padding:15px; border-radius:10px;">
                    <b>Múltiplos de 5:</b> 5, <span class="stp hl">10</span>, 15, 20...
                </div>
            </div>
            
            <div class="stp" style="margin-top:30px; font-size:36px; font-weight:900; color:#047857;">
                ¡El Mínimo Común Múltiplo es 10! ✅
            </div>
        </div>
    """)

    # Utilizando MCM para comparar (23)
    slide(f"""
        <div class="head-title">Resolviendo: 2/3 vs 3/4</div>
        <div style="background:#FFF9C4; padding:30px; border-radius:30px; border: 4px dashed #FBC02D; width:100%;">
            
            <div class="stp" style="background:#fff; padding:15px 30px; border-radius:15px; font-size:24px;">
                <b style="color:#F57F17;">Paso 1:</b> El MCM entre 3 y 4 es <b style="font-size:30px;">12</b>. Ambos denominadores deben ser 12.
            </div>
            
            <div class="stp" style="margin-top:15px; background:#fff; padding:15px 30px; border-radius:15px; font-size:24px;">
                <b style="color:#F57F17;">Paso 2:</b> Amplificamos (multiplicamos arriba y abajo) para llegar a 12:<br>
                2/3 (Multiplico x4) = <b>8/12</b><br>
                3/4 (Multiplico x3) = <b>9/12</b>
            </div>
            
            <div class="stp" style="margin-top:15px; background:#fff; padding:15px 30px; border-radius:15px; font-size:28px; text-align:center; box-shadow:0 5px 15px rgba(0,0,0,0.1);">
                <b style="color:#F57F17;">Paso 3:</b> ¡Ahora comparamos con sus nuevos cuerpos equivalente!<br>
                <b style="font-size:36px;">9/12 > 8/12</b> ➔ <b style="color:#2E7D32;">¡3/4 es mayor!</b>
            </div>
        </div>
    """)

    # Problemas Verbales Visiales (24-27)
    verbals = [
        ...
        ("Pelotas de Fútbol", "En una bolsa hay <b>5 pelotas</b>. <b>2</b> son verdes. ¿Qué fracción son las verdes?", "Son 2 de un total de 5 = <br><span style='font-size:46px; color:var(--base);'>2/5</span>", "⚽️", "#F8BBD0", "#C2185B")
    ]
    ...
    slide(f"""
        ...
        <div style="display:flex; flex-direction:column; gap:20px; width:100%;">
            ...
            <div class="stp pnl" style="padding:15px; font-size:20px;"><b style="font-size:30px">🔢</b> <b>Num/Denom:</b> Arriba las partes pintadas. Abajo el TOTAL de partes en la figura.</div>
            <div class="stp pnl" style="padding:15px; font-size:20px;"><b style="font-size:30px">⚖️</b> <b>Comparar Mismo Denom:</b> Mayor numerador gana (más trozos pintados).</div>
            ...
            <div class="stp pnl" style="padding:15px; font-size:20px;"><b style="font-size:30px">✖️</b> <b>MCM:</b> El Mínimo Común Múltiplo sirve para igualar los denominadores y así poder comparar con justicia.</div>
        </div>
    """)
    ...
    slide(f"""
        ...
        <div style="font-size:32px; font-weight:800; text-align:center; margin-top:20px;">
            ¡Hemos terminado la sección de fracciones básicas!<br>Todo este conocimiento es tuyo para siempre.
        </div>
...
"""
    with open('/Users/brunonattino/Desktop/PAGINA TUTORIAS/SLIDES/fracciones HTMLS/09-fracciones-1.html', 'w', encoding='utf-8') as f:
        f.write(html)
    print("Módulo 09 v4 generado! Slides:", len(slides))
